...
namespace memory
{
    ...
    bool file_shm_handler<MAPPED_FILE>::initial_shm(uint64_t full_file_size)
    {
        try {
            ...
            //TODO: Plan-00004 :Change to memory menagement size system.
            uint64_t shm_initial_size = full_file_size * EXTENED_SIZE_SHM;

						logger->write_info("file_shm_handler::initial_shm(), SHM-ID : ",
								shm_name);

            if(shm_name.empty()) {
                logger->write_info("file_shm_handler::initial_shm(), Name not support shm");
                return false;
            }
            ...
            file_shm = new boostinp::managed_shared_memory(
                    boostinp::create_only, shm_name.c_str(), shm_initial_size);
    ...
    template<typename MAPPED_FILE>
    bool file_shm_handler<MAPPED_FILE>::set_shm_name(std::string  uuid)
    {
        if(uuid.size() == 0) {
            return false;
        }
    ...
    template<typename MAPPED_FILE>
    const std::string file_shm_handler<MAPPED_FILE>::get_shm_name()
    {
        return shm_name;
    }
